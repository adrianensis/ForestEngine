...
def generate_cmake_data():
    ...
    log.log(log.LogLabels.build, system_info)
    ...
    log.log(log.LogLabels.build, "compilation_cores: " + str(compilation_cores))

    if system_name == "Linux" or system_name == "Linux2":
        # linux
        data.cmake_generator = '-G "Unix Makefiles"'
        data.systemBuildCommand = 'make -j{compilation_cores}'.format(compilation_cores = compilation_cores)
...
def build_cmake(projectDir, buildDir, buildType, cmake_generated_data: CMakeGeneratedData, buildCommandArgs):
    log.log(log.LogLabels.build, "-----------------------------------")
