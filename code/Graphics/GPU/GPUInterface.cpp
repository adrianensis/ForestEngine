...
void GPUInterface::enableTexture(u32 textureId, u32 textureUnit, GPUPipelineStage stage)
{
    ...
    CHECK_MSG(textureUnit < maxTextureUntis, "Max GPUTexture Unit reached!");
...
TextureHandle GPUInterface::getTextureHandle(u32 texture​Id)
{
    PROFILER_CPU()
    PROFILER_GPU()
    return 0;//glGetTextureHandleARB(texture​Id);
}
...
void GPUInterface::bindSharedBufferToShader(u32 programId, GPUBufferType bufferType, const HashedString& bufferName, u32 bindingPoint)
{
    ...
    switch (bufferType)
    {
        case GPUBufferType::UNIFORM:
        {
            u32 location = 0;//glGetUniformBlockIndex(programId, bufferName.get().c_str());   
            //glUniformBlockBinding(programId, location, bindingPoint);
            break;
        }
        case GPUBufferType::STORAGE:
        {
            u32 location = 0;//glGetProgramResourceIndex(programId, GL_SHADER_STORAGE_BLOCK, bufferName.get().c_str());
...
{
    // i32 errorCode = 0;//glCheckFramebufferStatus(GL_FRAMEBUFFER);
    // // checkGPUErrors();
	// bool errorDetected = false;
    // std::string errorMessage;
