...
void GPUBuffer::init(u32 attributeLocation, const GPUBufferData& data, bool isStatic)
{
    ...
    u32 sizeInPrimitiveTypes = mData.getSizePrimitiveType();
    if(mData.mAttributeDivisorSizeInPrimitiveTypes > 0)
    {
        u32 n = sizeInPrimitiveTypes/mData.mAttributeDivisorSizeInPrimitiveTypes;
        FOR_RANGE(i, 0, n)
        {
            attribute(mData.mPrimitiveType, mData.mAttributeDivisorSizeInPrimitiveTypes);
        }
    }
    else
    {
...
{
	GET_SYSTEM(RenderContext).resizeVBOAnyType(mVBO, mData.mTypeSizeInBytes, size, mIsStatic ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
}
...
u32 GPUBuffer::attribute(GPUBufferPrimitiveType primitiveType, u32 customSizeInPrimitiveTypes /*=0*/)
{
    ...
    GET_SYSTEM(RenderContext).attribute(getAttributeLocationWithOffset(), sizeInPrimitiveTypes, static_cast<u32>(primitiveType), mData.mTypeSizeInBytes, mPreviousOffsetInBytes, mData.mInstanceDivisor);
    // accumulative offset in bytes
    u32 primitiveTypeSizeInBytes = mData.getPrimitiveTypeSizeInBytes();
