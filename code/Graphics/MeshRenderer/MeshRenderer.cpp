...
{
    PROFILER_CPU()
    Maths::Matrix4 rendererModelMatrix = GET_SYSTEM(EC::EntityComponentManager).getFirstComponent<Transform>(getOwnerEntity())->calculateModelMatrix();
...
void MeshRenderer::update()
{
    ...
    {
        if(GET_SYSTEM(EC::EntityComponentManager).getFirstComponent<Transform>(getOwnerEntity())->getModelMatrixDirty())
        {
            calculateRendererModelMatrix();
        }
    }
