#pragma once

#include "Core/BasicTypes.hpp"
#include "Core/Serialization.hpp"
#include "Core/Maths/VectorDef.hpp"

#ifdef CPP_INCLUDE
#include "Core/Maths/Vector3.hpp"
#include "Core/Maths/Vector2.hpp"
#include "Core/Maths/Vector4.hpp"
#include "Core/Maths/MathUtils.hpp"
#endif

class Vector2;
class Vector4;

class Vector3
{
public:

	f32 x = 0;
	f32 y = 0;
	f32 z = 0;
	
	VECTOR_BASE_DEFINITION(3)

	CPP Vector3() : x(0), y(0), z(0)
	{
	}

	CPP Vector3(f32 x, f32 y, f32 z) : x(x), y(y), z(z)
	{
	}

	CPP Vector3(const Vector3& other) : x(other.x), y(other.y), z(other.z)
	{
	}

	CPP Vector3(const Vector2& other) : x(other.x), y(other.y), z(0.0f)
	{
	}

	CPP Vector3(const Vector2& other, f32 z) : x(other.x), y(other.y), z(z)
	{
	}

	CPP Vector3(const Vector4& other) : x(other.x), y(other.y), z(other.z)
	{
	}

	CPP Vector3& set(f32 x, f32 y, f32 z)
	{
		this->x = x;
		this->y = y;
		this->z = z;
		return *this;
	}

	CPP Vector3& set(const Vector2& rhs)
	{
		this->set(rhs.x, rhs.y, 0);
		return *this;
	}

	CPP Vector3& set(const Vector3& rhs)
	{
		this->set(rhs.x, rhs.y, rhs.z);
		return *this;
	}

	CPP Vector3& set(const Vector4& rhs)
	{
		this->set(rhs.x, rhs.y, rhs.z);
		return *this;
	}

	CPP Vector3& add(const Vector3& rhs)
	{
		// can be parallelized with SIMD auto-vectorization
		x = x + rhs.x;
		y = y + rhs.y;
		z = z + rhs.z;
		return *this;
	}

	CPP Vector3& sub(const Vector3& rhs)
	{
		x = x - rhs.x;
		y = y - rhs.y;
		z = z - rhs.z;
		return *this;
	}

	CPP Vector3& mul(const Vector3& rhs)
	{
		x = x * rhs.x;
		y = y * rhs.y;
		z = z * rhs.z;
		return *this;
	}

	CPP Vector3& div(const Vector3& rhs)
	{
		ASSERT_MSG(rhs.x != 0, "Division by zero.");
		ASSERT_MSG(rhs.y != 0, "Division by zero.");
		ASSERT_MSG(rhs.z != 0, "Division by zero.");
		x = x / rhs.x;
		y = y / rhs.y;
		z = z / rhs.z;
		return *this;
	}

	CPP Vector3& add(f32 rhs)
	{
		x = x + rhs;
		y = y + rhs;
		z = z + rhs;
		return *this;
	}

	CPP Vector3& sub(f32 rhs)
	{
		x = x - rhs;
		y = y - rhs;
		z = z - rhs;
		return *this;
	}

	CPP Vector3& mul(f32 rhs)
	{
		x = x * rhs;
		y = y * rhs;
		z = z * rhs;
		return *this;
	}

	CPP Vector3& div(f32 rhs)
	{
		ASSERT_MSG(rhs != 0, "Division by zero.");
		x = x / rhs;
		y = y / rhs;
		z = z / rhs;
		return *this;
	}

	CPP f32 dot(const Vector3& v) const
	{
		return this->x * v.x + this->y * v.y + this->z * v.z;
	}

	CPP f32 sqrlen() const
	{
		return this->dot(*this);
	}

	CPP f32 sqrdst(const Vector3& v) const
	{
		Vector3 sub = Vector3(v) - (*this);
		return sub.dot(sub);
	}

	CPP f32 len() const
	{
		return sqrtf(this->sqrlen());
	}

	CPP f32 max() const
	{
		return std::max(x, std::max(y, z));
	}

	CPP f32 min() const
	{
		return std::min(x, std::min(y, z));
	}

	CPP Vector3& nor()
	{
		f32 len = this->len();

		//ASSERT_MSG(len > 0, "Length is zero.");

		if (len > 0)
		{
			this->div(len);
		}

		return *this;
	}

	CPP f32 dst(const Vector3& v) const
	{
		return sqrtf(this->sqrdst(v));
	}

	CPP bool eq(const Vector3& v) const
	{
		return MathUtils::eqf(this->x, v.x) && MathUtils::eqf(this->y, v.y) && MathUtils::eqf(this->z, v.z);
	}

	CPP bool eq(const Vector3& v, f32 e) const
	{
		return MathUtils::eqf(this->x, v.x, e) && MathUtils::eqf(this->y, v.y, e) && MathUtils::eqf(this->z, v.z, e);
	}

	CPP Vector3& cross(const Vector3& v)
	{
		f32 x1 = this->x;
		f32 y1 = this->y;
		f32 z1 = this->z;

		f32 x2 = v.x;
		f32 y2 = v.y;
		f32 z2 = v.z;

		this->x = y1 * z2 - z1 * y2;
		this->y = z1 * x2 - x1 * z2;
		this->z = x1 * y2 - y1 * x2;

		return *this;
	}

	CPP Vector3& lerp(const Vector3& target, f32 t)
	{
		//start + percent*(end - start)
		(*this) += (Vector3(target) - (*this)) * t;
		return *this;
	}

	CPP Vector3& nlerp(const Vector3& target, f32 t)
	{
		//start + percent*(end - start)
		this->lerp(target, t).nor();
		return *this;
	}

	CPP Vector3& slerp(const Vector3& target, f32 t)
	{
		if (t == 0)
			return *this;

		f32 theta = angle(target);
		//f32 thetaDegree = theta * RadianToDegree;

		f32 sinTheta = sinf(theta);

		return this->mul(sinf((1 - t) * theta) / sinTheta).add(target * (sinf(t * theta) / sinTheta));
	}

	CPP f32 angle(const Vector3& v) const
	{
		/*
		* angle is acute (positive dot product)
		* perpendicular (zero dot product)
		* or obtuse (negative dot product).
		*/
		// between 0 - PI (radians)
		return acosf(this->dot(v) / (this->len() * v.len()));
	}

	CPP f32 angle(const Vector3& v, const Vector3& n) const
	{
		f32 dot = this->dot(v);
		Vector3 cross = Vector3(*this).cross(v);
		f32 radians = atan2f(cross.len(), dot);

		radians = n.dot(cross) < 0.0f ? (2.0f * MathUtils::PI) - radians : radians;

		return radians;
	}

	CPP Vector3& clamp(f32 maxLength)
	{
		if (this->sqrlen() > maxLength* maxLength)
		{
			this->nor();
			this->mul(maxLength);
		}

		return *this;
	}
};

TEMPLATED_SERIALIZATION(Vector3)

#ifdef CPP_INCLUDE
template<>
JSON SerializationUtils::serializeTemplated(const Vector3& value)
{
	JSON json;
	DO_SERIALIZE("x", value.x)
	DO_SERIALIZE("y", value.y)
	DO_SERIALIZE("z", value.z)
	return json;
}

template<>
void SerializationUtils::deserializeTemplated(Vector3& value, const JSON& json)
{
	DO_DESERIALIZE("x", value.x)
	DO_DESERIALIZE("y", value.y)
	DO_DESERIALIZE("z", value.z)
}
#endif