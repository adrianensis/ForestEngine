...
private:
	...
	CPP void setRows(const std::array<f32, 4> &row0, const std::array<f32, 4> &row1, const std::array<f32, 4> &row2, const std::array<f32, 4> &row3)
	{
	...
	CPP void setRows(const f32 *row0, const f32 *row1, const f32 *row2, const f32 *row3)
	{
	...
	CPP void setRows(u32 n)
	{
		FOR_RANGE(i, 0, 4 * 4)
		mData[i] = n;
	}

	CPP void setRows(const Vector4& row0, const Vector4& row1, const Vector4& row2, const Vector4& row3)
	{
...
public:
	...
	CPP static const Matrix4& getIdentity()
	{
		if (!smIdentity)
		{
	...
	CPP void init(const Matrix4& other)
	{
		init(other.mData); //TRACE()
	}

	CPP void init(const std::array<f32, 16> &data)
	{
		memcpy(mData, &data[0], 16 * sizeof(f32));
	}

	CPP void init(const std::array<f32, 4> &row0, const std::array<f32, 4> &row1, const std::array<f32, 4> &row2, const std::array<f32, 4> &row3)
	{
		setRows(row0, row1, row2, row3);
	}

	CPP void init(const f32 *data)
	{
		memcpy(mData, data, 16 * sizeof(f32));
	}

	CPP void init(const f32 *row0, const f32 *row1, const f32 *row2, const f32 *row3)
	{
		setRows(row0, row1, row2, row3);
	}

	CPP void init(const Vector4& row0, const Vector4& row1, const Vector4& row2, const Vector4& row3)
	{
		setRows(row0, row1, row2, row3);
	}
	...
	CPP f32 get(u8 row, u8 col) const
	{
		return mData[row + (4 * col)];
	}

	CPP void set(u8 row, u8 col, f32 value)
	{
	...
	CPP void mul(const Matrix4& other)
	{
		Matrix4 copy;
		copy.init((*this));
	...
	CPP Vector4 mulVector(const Vector4& vector) const
	{
	...
	CPP void translation(const Vector3& vector)
	{
		this->identity();
	...
	CPP void rotation(const Vector3& vector)
	{
		Quaternion q(vector);
		q.toMatrix(this);
	...
	CPP void scale(const Vector3& vector)
	{
		this->identity();
	...
	CPP void ortho(f32 left, f32 right, f32 bottom, f32 top, f32 near, f32 far)
	{
		this->identity();
	...
	CPP void perspective(f32 near, f32 far, f32 aspect, f32 fov)
	{
		f32 zRange = far - near;
